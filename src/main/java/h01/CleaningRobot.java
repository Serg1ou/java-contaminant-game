...
public class CleaningRobot extends Robot implements Cleaner, TickBased {
    ...
    public void handleKeyInput(
        final int direction,
        final boolean shouldPutCoins,
        final boolean shouldPickCoins
    ) {

        // Pick up a coin if possible and capacity allows it
        if (shouldPickCoins
            && isOnACoin()
            && getNumberOfCoins() < GameConstants.CLEANER_CAPACITY) {
            pickCoin();
        }
        // Otherwise, put a coin if requested
        else if (shouldPutCoins && hasAnyCoins()) {
            putCoin();
        }

        // Handle movement based on direction input
        if (direction == 0) { // UP
            while (!isFacingUp()) {
                turnLeft();
            }
